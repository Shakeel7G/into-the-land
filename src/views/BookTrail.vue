...
async function loadAreasAndTrails() {
  try {
    ...
    // Preselect from query or pending selection saved in localStorage
    const trailIdParam = route.query.trail;
    const areaParam = route.query.area;
    const pending = (() => {
      try { return JSON.parse(localStorage.getItem('pendingTrail')); } catch (e) { return null; }
    })();

    // prefer explicit query params, fall back to pending selection
    const trailIdToUse = trailIdParam || pending?.id;
    const areaToUse = areaParam || pending?.area;

    if (areaToUse) {
      const areaObj = areas.value.find(a => a.name.toLowerCase() === decodeURIComponent(areaToUse).toLowerCase());
      if (areaObj) {
        selectedArea.value = areaObj;
        availableTrails.value = areaObj.trails;
        if (trailIdToUse) selectedTrail.value = areaObj.trails.find(t => t.id == trailIdToUse) || "";
      }
    } else if (trailIdToUse) {
      for (const areaObj of areas.value) {
        const trailObj = areaObj.trails.find(t => t.id == trailIdToUse);
        if (trailObj) {
          selectedArea.value = areaObj;
          availableTrails.value = areaObj.trails;
    ...
    // Remove pending once used
    if (pending) localStorage.removeItem('pendingTrail');
  } catch (error) {
    console.error(error);
